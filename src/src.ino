...
char codeVersion[] = "0.16-beta.2rw";  // Software revision.
...
int numberOfChannels = sizeof(servopin) / sizeof(uint8_t);                                                              // for PWM Reader
...
int pwmFreq = 0;
bool selected_servo_has_changed = true;
...
// for auto calibration neutral position set #define AUTO_ZERO_CALIBRATION true
            ...
            // When the menu changes to another channel, this channel can be min/max calibrated in the next 5 sec.
            if (selected_servo_has_changed && selectedServo < 5) {
                pwm_set_auto_min_max(selectedServo, true);         // auto min/max calibration in the next 5 sec
                pwm_set_state(selectedServo, PwmState::UNSTABLE);  // reset status to enable a new calibration
                selected_servo_has_changed = false;
            }
            ...
            if (millis() - pwmMenuMillis > 100) {  // Every 100ms (slow screen refresh down, display is too nervous otherwise!)
                ...
                display.clear();
                if (selectedServo >= 5) {                     //-- GraphOverview
                    selectedServo = 5;                        // 5 means GraphOverview of all channels

                    int32_t channelIndexOfMinimumOffset = 0;  // this channel is drawn at x0
                    int32_t minimumOffset = 0;
                    int32_t maximumImpulseWidth = 2000;
                    int32_t maximumOffset = 0;
                    for (uint8_t ch = 0; ch < numberOfChannels; ch++) {  //-- get min/max values for scale factor and pulse draw
                        if (pwm_get_state(ch) == PwmState::STABLE) {     // only consider stable channels
                            int32_t offset = pwm_get_offset(ch);
                            if (offset < minimumOffset) {
                                minimumOffset = offset;
                                channelIndexOfMinimumOffset = ch;
                            }
                            maximumImpulseWidth = MAX(pwm_get_rawPwm(ch), maximumImpulseWidth);
                            maximumOffset = MAX(abs(offset), maximumOffset);
                        }
                    }
                    //-- make GraphOverview
                    int rowHigh = DISPLAY_HEIGHT / numberOfChannels;
                    int firstRow = ((DISPLAY_HEIGHT - rowHigh * numberOfChannels) / 2);                             // in this case: (64 - (12*5)/2 = 2
                    int channelHigh = rowHigh - firstRow - 1;                                                       // 2px for lower edge 1px distance between channels (Drawing height of the channel)
                    int maxDrawWidth = maximumOffset + maximumImpulseWidth;                                         // The drawing width is adjusted dynamically
                    float scaleFactor = static_cast<float>(DISPLAY_WIDTH - 10) / static_cast<float>(maxDrawWidth);  // 10px for left lables
                    display.setTextAlignment(TEXT_ALIGN_LEFT);
                    display.setFont(ArialMT_Plain_10);

                    for (uint8_t ch = 0; ch < numberOfChannels; ch++) {
                        int scaledMaxDrawWidth = maxDrawWidth * scaleFactor;
                        auto scaledRawPwm = pwm_get_rawPwm(ch) * scaleFactor;

                        int x0 = 10;                  // x-start for drawing
                        int y0 = rowHigh * (ch + 1);  // y-start for drawing

                        //-- draw channel number on left side
                        display.drawString(0, firstRow + (rowHigh * ch), String(ch + 1));

                        if (pwm_get_state(ch) != PwmState::STABLE) {  // if channel not STABLE draw a 0-line
                            display.drawHorizontalLine(x0, y0, scaledMaxDrawWidth);
                            continue;
                        }

                        if (ch == channelIndexOfMinimumOffset) {  // the channel drawn on the leftmost side without offset
                            // impuls without offset
                            display.drawVerticalLine(x0, y0 - channelHigh, channelHigh);                 // vertical 9px
                            display.drawHorizontalLine(x0, y0 - channelHigh, scaledRawPwm);              // horizontal rawPwm
                            display.drawVerticalLine(x0 + scaledRawPwm, y0 - channelHigh, channelHigh);  // vertical -9px

                            // rest
                            display.drawHorizontalLine(x0 + scaledRawPwm, y0, scaledMaxDrawWidth - scaledRawPwm);  // horizontal maxDrawWidth - rawPwm
                        } else {
                            // offset
                            auto scaledOffset = abs((abs(minimumOffset) - abs(pwm_get_offset(ch)))) * scaleFactor;
                            display.drawHorizontalLine(x0, y0, scaledOffset);

                            // impuls
                            display.drawVerticalLine(x0 + scaledOffset, y0 - channelHigh, channelHigh);                 // vertical 9px
                            display.drawHorizontalLine(x0 + scaledOffset, y0 - channelHigh, scaledRawPwm);              // horizontal rawPwm
                            display.drawVerticalLine(x0 + scaledOffset + scaledRawPwm, y0 - channelHigh, channelHigh);  // vertical -9px

                            // rest
                            display.drawHorizontalLine(x0 + scaledOffset + scaledRawPwm, y0, scaledMaxDrawWidth - scaledRawPwm - scaledOffset);  // horizontal maxDrawWidth - rawPwm
                        }                                                                                                                        // display.drawHorizontalLine(10, rowHigh * (ch + 1), scaledMaxDrawWidth);
                    }

                } else {                                                                         // individual channel
                    auto config = pwm_get_channel_config(selectedServo);                         // pointer to channels config struct
                    servo_pos[selectedServo] = static_cast<int>(pwm_get_rawPwm(selectedServo));  // read PWM signal
                    pwmFreq = static_cast<int>(pwm_get_frequency(selectedServo));                // read PWM frequency
                    auto offset = pwm_get_offset(selectedServo);                                 // read PWM offset
                    const PwmState state = pwm_get_state(selectedServo);                         // read PWM state
                    int Impuls_min = static_cast<int>(config->pulse_min);                        // read pulse_min
                    int Impuls_max = static_cast<int>(config->pulse_max);                        // read pulse_max
// for auto measured min/max
#if AUTO_MIN_MAX_CALIBRATION == false
                    // measure min/max automatically continuously
                    if (state == PwmState::STABLE) {
                        Impuls_min = (servo_pos[selectedServo] > 0)
                                         ? MIN(Impuls_min, servo_pos[selectedServo])
                                         : Impuls_min;

                        Impuls_max = (servo_pos[selectedServo] > 0)
                                         ? MAX(Impuls_max, servo_pos[selectedServo])
                                         : Impuls_max;

                        pwm_set_channel_pulse_min(selectedServo, static_cast<uint16_t>(Impuls_min));
                        pwm_set_channel_pulse_max(selectedServo, static_cast<uint16_t>(Impuls_max));
                    }
                    // end measure min/max
#endif
                    float_t rcDc = pwm_get_rc_duty_cycle(Impuls_min, Impuls_max, servo_pos[selectedServo]);  // read PWM Duty Cycle
                    char str[25];                                                                            // max. place for sprintf()

                    // channel
                    display.setTextAlignment(TEXT_ALIGN_LEFT);
                    display.setFont(ArialMT_Plain_16);
                    display.drawString(0, 0, impulseString[LANGUAGE] + String(selectedServo + 1));

                    // impulse length
                    display.setTextAlignment(TEXT_ALIGN_RIGHT);
                    display.drawString(55, 17, String(servo_pos[selectedServo]) + "µs");

                    // freq, offset, rcdc:
                    display.setTextAlignment(TEXT_ALIGN_RIGHT);
                    display.setFont(ArialMT_Plain_10);
                    sprintf(str, "frq:%4d Hz", pwmFreq);
                    display.drawString(128, 0, str);

                    sprintf(str, "offset: %4ld µs", offset);
                    display.drawString(128, 12, str);

                    sprintf(str, "dc: %4.0f %%", rcDc);
                    display.drawString(128, 24, str);

                    // print progress bar or status
                    switch (state) {
                        case PwmState::STABLE: {
                            if ((Impuls_max - Impuls_min) > 100) { //if you work with AUTO_MIN_MAX_CALIBRATION, min/max are equal at the beginning of the measurement
                                // Impulse_min/-max
                                display.setTextAlignment(TEXT_ALIGN_LEFT);
                                display.setFont(ArialMT_Plain_10);
                                display.drawString(0, 39, String(Impuls_min));
                                display.setTextAlignment(TEXT_ALIGN_RIGHT);
                                display.drawString(128, 39, String(Impuls_max));
                        ...
                        default: {
                            ...
                            display.setTextAlignment(TEXT_ALIGN_CENTER);
                            display.setFont(ArialMT_Plain_10);
                            String statusString = (state == PwmState::UNSTABLE)
                                                      ? impulseStateUnstableString[LANGUAGE]
                                                      : impulseStateNotConnectedString[LANGUAGE];
                            display.drawString(64, 50, statusString);
                        }
                    }
                }
                display.display();
            ...
            if (encoderState == 1) {
                selectedServo--;
                selected_servo_has_changed = true;
            }
        ...
        // Multiswitch lesen *********************************************************
        // www.modelltruck.net/showthread.php?54795-Futaba-Robbe-Multiswitch-Decoder-mit-Arduino
